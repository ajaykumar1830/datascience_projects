...
model <- glm (TenYearCHD ~.-Age_bins, data = train_data, family = binomial)
...
train_data$prob_train <- predict(best_model, type="response")
View(train_data)
...
xgb.fit <- xgboost(
  data = features_train,
  label = label_train,
  eta = 0.01,#this is like shrinkage in the previous algorithm
...
tapply(predictTrain_xgb, train_data$TenYearCHD, mean)
# Confusion matrix for threshold of 0.45
table(train_data$TenYearCHD, predictTrain_xgb >= 0.46)
test_data$xgb.pred.class <- predict(xgb.fit, features_test)
table(test_data$TenYearCHD, test_data$xgb.pred.class >= 0.46)
Accuracy = (164+408)/(164+408+671+29)
Accuracy 
Senstivity <- 164/(164+29)
Senstivity
Precision <- 164/(671)
Precision
```
